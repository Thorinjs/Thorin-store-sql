    ...
    Object.keys(models).forEach((key) => {
      ...
      Object.keys(modelObj.fields).forEach((fieldName) => {
        ...
        if(modelObj.getters[fieldName]) {
          fieldOpt.get = modelObj.getter[fieldName];
        }
        if(modelObj.setters[fieldName]) {
          fieldOpt.set = modelObj.setter[fieldName];
        }
      ...
      options.validate = modelObj.validations;
      var modelInstance;
      try {
        modelInstance = seqObj.define(modelObj.code, attributes, options);
      } catch(e) {
        console.error('Thorin.sql: could not create sequelize schema for: ' + modelObj.path + '\n', e.stack);
        return;
      }
      modelObj._setInstance(modelInstance);
    });
    done();
  }
