...
module.exports = function init(thorin) {
  ...
  * */
  function createInstances(privateAttributes, seqObj, models, done) {
    Object.keys(models).forEach((key) => {
      ...
      if(options.updatedAt === true) options.updatedAt = 'updated_at';
      if(options.deletedAt === true) options.deletedAt = 'deleted_at';
      // map the attributes
      Object.keys(modelObj.fields).forEach((fieldName) => {
        ...
        if(fieldOpt.private) {
          privateAttributes.push(fieldName);
        }
      ...
      function ToJSON(jsonName) {
        ...
        if(typeof result === 'object' && result != null) {
          for(let i=0; i < privateAttributes.length; i++) {
            if(typeof result[privateAttributes[i]] !== 'undefined') {
              delete result[privateAttributes[i]];
            }
          }
        }
        return result;
  ...
  * */
  function createAssociations(privateAttributes, models, done) {
    Object.keys(models).forEach((key) => {
      let modelObj = models[key],
      ...
      modelObj.relations.forEach((relation) => {
        ...
        let opt = thorin.util.extend(getAssociationOptions(targetModel.tableName, relation.options),{
          as: modelObj.code
        }, relation.options);
        relation.options = opt;
        if(opt.private) {
          privateAttributes.push(opt.foreignKey.name);
        }
        try {
  ...
  * */
  loader.finalizeLoading = function initialize(storeObj, config, models, onDone) {
    ...
    calls.push((done) => {
      createInstances(privateAttributes, seqObj, models, done);
    });

    /* step 4: create associations */
    calls.push((done) => {
      createAssociations(privateAttributes, models, done);
    });
  ...
  /* PRIVATE */
  function getAssociationOptions(name, opt) {
    let options = {
      onDelete: 'CASCADE',
      onUpdate: 'CASCADE'
    };
    ...
    if(!opt.foreignKey) {
      options.foreignKey = {
        name: decamelize((opt.as || name) + "Id", '_'),
        allowNull: canBeNull
      };
