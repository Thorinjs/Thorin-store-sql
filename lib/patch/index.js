        ...
        for (let j = 0, jlen = statements.length; j < jlen; j++) {
          ...
          if (isApplied) {
            ...
            try {
              ...
              logger.fatal(`Failed to apply patch: [${patch.text_version}] statement ${j + 1}/${statements.length} - ${e.message}`);
              if (store._hasFkCheck()) {
                await runSql('SET FOREIGN_KEY_CHECKS=1;', true);
              }
              throw thorin.error('SQL.PATCH', `Failed to apply patch ${patch.text_version} statement ${j + 1}/${statements.length}: [${e.message}]`);
            }
          }
        }
      }
