...
module.exports = (thorin, opt, Store, storeObj, config) => {
  ...
  function prepareModel(modelObj) {
    ...
    function onSave(iObj, opt, fn) {
      ...
      for (let i = 0, len = encrypted.length; i < len; i++) {
        ...
        try {
          if (iObj._changed?.has) { // sequelize v6
            if (!iObj._changed.has(fieldName)) continue;
          } else if (!iObj._changed[fieldName]) { // older object/map.
            continue;
          }
        } catch(e) {
        }
  ...
  thorin.on('PRE_RUN', `store.${storeObj.name}`, () => {
    _prepare();
  });
