...
module.exports = (thorin, opt, storeObj, config) => {
  ...
  function isEnabled() {
    ...
    if (typeof config !== 'object' || !config) {
      logger.warn(`Crypto config is missing`);
      return false;
    }
    if (typeof config.key !== 'string' && typeof config.keys === 'undefined') {
      logger.info(`No crypto keys found. Crypto is disabled`);
      return false;
    }
    if (!CURRENT_KEY) {
      logger.info(`Crypto does not have a key. Feature is disabled`);
      return false;
    }
    return true;
  }
  ...
  if (CURRENT_KEY) {
    currentKeyVersion = getKeyVersion(CURRENT_KEY);
  ...
  class SqlCryptor {
    ...
    /*
    * Generates the store-sql key version for the given key.
    * */
    getKeyVersion(key) {
      if (typeof key !== 'string' || !key) return null;
      return getKeyVersion(key, true);
    }
     ...
     *  }
     * */
    encrypt(value, key, forceKey) {
      if (!CURRENT_KEY && !forceKey) return value;
      try {
     ...
     *  the appropiate key.
     * */
    decrypt(value, key, forceKey) {
      if (!CURRENT_KEY && !forceKey) return value;
      if (typeof value !== 'string' || !value) return null;
      if (!this.isEncrypted(value) && !forceKey) return value;
  ...
  storeObj.crypto.active = isEnabled();
  return storeObj.crypto.active;
};
