...
module.exports = function(thorin) {
  ...
  actions.update = function(actionObj, opt) {
    ...
    var aliases = getAliases(actionObj, 'POST', '/' + modelObj.code + '/:' + primaryKey.fieldName),
      namespace = opt.namespace || ACTION_NAMESPACE,
      actionId = opt.action || (namespace + '.update.' + modelObj.code);
    var dbAction = new thorin.Action(actionId);
    attach(actionObj, dbAction, aliases);
    let fieldValidator = {},
      foreignKeys = [];
  ...
  actions.delete = function(actionObj, opt) {
    ...
    var aliases = getAliases(actionObj, 'DELETE', '/' + modelObj.code + '/:' + primaryKey.fieldName),
      namespace = opt.namespace || ACTION_NAMESPACE,
      actionId = opt.action || (namespace + '.delete.' + modelObj.code);
    var dbAction = new thorin.Action(actionId);
    attach(actionObj, dbAction, aliases);
