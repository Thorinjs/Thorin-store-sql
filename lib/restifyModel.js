 ...
 *  create.after(instanceObj, savedInstanceObj)
...
module.exports = function(thorin) {
  ...
  actions.create = function(actionObj, opt) {
    ...
    var aliases = getAliases(actionObj, 'POST', '/' + opt.name),
      namespace = opt.namespace || ACTION_NAMESPACE,
      actionId = opt.action || (namespace + '.' + opt.name + '.create');
    ...
    modelObj.relations.forEach((relation) => {
      if(relation.options && relation.options.private) return;
  ...
  actions.read = function(actionObj, opt) {
    ...
    var aliases = getAliases(actionObj, 'GET', '/' + opt.name + '/:' + primaryKey.fieldName),
      namespace = opt.namespace || ACTION_NAMESPACE,
      actionId = opt.action || (namespace + '.' + opt.name + '.read');
    var dbAction = new thorin.Action(actionId);
    attach(actionObj, dbAction, aliases);
    let fieldValidator = {};
    fieldValidator[primaryKey.fieldName] = dispatcher.validate(primaryKey.type.key);
    fieldValidator[primaryKey.fieldName].error(thorin.error('INPUT.NOT_VALID', 'Invalid or missing ' + primaryKey.fieldName));
    dbAction.input(fieldValidator);
    dbAction.use((intentObj) => {
      let DbModel = this.model(modelObj.code),
        findQuery = {
  ...
  actions.find = function(actionObj, opt) {
    var modelObj = actionObj.model;
    var aliases = getAliases(actionObj, 'GET', '/' + opt.name),
      namespace = opt.namespace || ACTION_NAMESPACE,
      actionId = opt.action || (namespace + '.' + opt.name + '.find');
    var dbAction = new thorin.Action(actionId);
    attach(actionObj, dbAction, aliases);
    let fieldValidator = {},
      attributeFilters = [];
  ...
  actions.update = function(actionObj, opt) {
    var modelObj = actionObj.model;
    var primaryKey = modelObj.getPrimary();
    if (!primaryKey) {
      console.warn('Thorin.sql.restify: model ' + modelObj.code + ' does not have a primary key for "update" restify');
      return;
    }
    var aliases = getAliases(actionObj, 'POST', '/' + opt.name + '/:' + primaryKey.fieldName),
      namespace = opt.namespace || ACTION_NAMESPACE,
      actionId = opt.action || (namespace + '.' + opt.name + '.update');
    var dbAction = new thorin.Action(actionId);
    attach(actionObj, dbAction, aliases);
    let fieldValidator = {},
      foreignKeys = [];
    ...
    dbAction.use((intentObj) => {
      ...
      findQry.where[primaryKey.fieldName] = intentObj.input(primaryKey.fieldName);
  ...
  actions.delete = function(actionObj, opt) {
    ...
    var aliases = getAliases(actionObj, 'DELETE', '/' + opt.name + '/:' + primaryKey.fieldName),
      namespace = opt.namespace || ACTION_NAMESPACE,
      actionId = opt.action || (namespace + '.' + opt.name + '.delete');
    var dbAction = new thorin.Action(actionId);
    attach(actionObj, dbAction, aliases);
