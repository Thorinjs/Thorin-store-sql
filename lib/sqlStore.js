...
module.exports = function(thorin, opt) {
  ...
  const ERROR_NOT_CONNECTED = thorin.error('SQL.NOT_CONNECTED', 'Database connection not active', 502);
    ...
    init(storeConfig) {
      ...
      if (this.#config.storeConfig) {
        Object.keys(this.#config.storeConfig).forEach(c => {
          if (Sequelize[c]) {
            throw thorin.error('SQL.CONFIG', `Store config key [${c}] already present in Sequelize.`);
          }
