'use strict';
/* This is an ugly work-around sequelize's deprecation of string-based operators.
 * We cannot override the deprecation function, thus we will override the util.depreacate() function. */
const util = require('util'), _deprecate = util.deprecate, noop = () => {
};
util.deprecate = function (fn, msg, code) {
  if (code === 'SEQUELIZE0003') return noop;
  return _deprecate.apply(this, arguments);
};
const path = require('path'), exec = require('child_process').exec, decamelize = require('decamelize'), camelize = require('camelize'), initLoader = require('./loader'),
  initCrudify = require('./crudifyModel'), initModel = require('./storeModel'), initPatch = require('./patch/index'), initCrudifyAction = require('./crudifyAction'),
  Sequelize = require('sequelize');

const IGNORE_QUERIES = {
  sqlite: ['SET FOREIGN', 'ALTER DATABASE', 'SHOW']
};

try {
  // Hide Operator Aliases deprecation
  const deprecations = require('sequelize/lib/utils/deprecations');
  deprecations.noStringOperators = noop;
} catch (e) {
}
...
module.exports = function (thorin, opt) {
  const fse = thorin.util.fs;
  let logger = thorin.logger(opt.name), crudify = initCrudify(thorin), CrudifyAction = initCrudifyAction(thorin), SqlModel = initModel(thorin), loader = null;

  const ERROR_NOT_CONNECTED = thorin.error('STORE.NOT_CONNECTED', 'Database connection not active', 502);
  ...
  class ThorinSqlStore extends thorin.Interface.Store {
    ...
    #initialized = false;
    #config = {};
    #preConfig = {};   // config that was set before the init()
    #loaded = false;
    #models = {};     // map of modelName:modelObj
    #seq = null;
    #onBeforeSetup = null; // callback function to be called right before setup that has access to config.
    ...
    /**
     * Returns all our registered models.
     * */
    getModels() {
      return this.#models;
    }
    ...
    /**
     * Returns the SQL dialect we're using
     * */
    getDialect() {
      return this.#config.options.dialect;
    }

    /**
     * Given a Model object, it will retrieve an array with all its attributes, excluding the ones given in the second array
     *
     * */
    getModelAttributes(Model, exclude = []) {
      if (typeof Model === 'string') Model = this.model(Model);
      const res = [];
      try {
        const all = Object.keys(Model.attributes || {});
        for (let i = 0, len = all.length; i < len; i++) {
          const a = all[i];
          if (exclude.indexOf(a) !== -1) continue;
          res.push(a);
        }
      } catch (e) {
      }
    ...
    /**
     * Registers a callback function to be run before setup, to hook up to configs.
     * */
    onBeforeSetup(fn) {
      if (typeof fn !== 'function') return;
      this.#onBeforeSetup = fn;
    }
    ...
    init(storeConfig) {
      this.#config = thorin.util.extend({
        debug: {      // Setting this to =false, will disable debugging all together.
          create: true, read: true, update: true, delete: true, transaction: false, // Transaction initialization information
          crudify: (thorin.env === 'development')
        }, clean: (thorin.env === 'development'),  // if set to true, will drop create all tables
        host: 'localhost', port: null, user: null, password: null, database: null, path: {
          models: path.normalize(thorin.root + '/app/models'),   // the store model definition folder
          patch: path.normalize(thorin.root + '/sql/patch'), // the store sql files that will be executed.
          scripts: path.normalize(thorin.root + '/app/setup/scripts') // the node.js setup scripts that will be run as await scriptFn(thorin, transaction)
        }, patch: {
          tableName: '_system_version', delimiter: ';'    // the statement delimiter in patch files.
        }, sync: true,   // setting this to false will NOT sync the table models of the app.
        ...
        replication: null, tablePrefix: null,  // if specified, we will prefix this to all table names.
        options: {  // sequelize options
          collate: 'utf8_unicode_ci', charset: 'utf8', dialect: 'mysql', timezone: '+00:00', pool: {
            idle: 12000
          }, dialectOptions: {
            charset: 'utf8mb4'
          }
        }, storeConfig: {} // Additional static config to be attached to Seq.{key}
      }, storeConfig);
      ...
      if (this.#config.storeConfig) {
        Object.keys(this.#config.storeConfig).forEach(c => {
          if (Sequelize[c]) {
            throw thorin.error('STORE.CONFIG', `Store config key [${c}] already present in Sequelize.`);
          }
          Sequelize[c] = this.#config.storeConfig[c];
        });
      }
      ...
      if (replica != null) {
        ...
        for (let i = 0; i < replica.length; i++) {
          ...
          repl.read.push(item);
          repl.write = {
            host: this.#config.host, port: this.#config.port, username: this.#config.username, password: this.#config.password, pool: this.#config.options.pool || {}
          };
        }
      ...
      if (this.#config.path.patch && !path.isAbsolute(this.#config.path.patch)) {
        this.#config.path.patch = path.normalize(thorin.root + '/' + this.#config.path.patch);
      }
      ...
      this.patch = initPatch(thorin, this.#config, this);
     ...
     * */
    async getTables() {
      let items = await this.query('SHOW TABLES IN ' + this.#config.database, { logging: false });
      const tables = items[0];
      const res = [];
      tables.forEach(t => {
    ...
    async run(done) {
      ...
      opt.operatorsAliases = operatorsAliases;
      this.#seq = new Sequelize(this.#config.database, this.#config.user || null, this.#config.password || null, opt);
      if (this.#config.storeConfig) {
        Object.keys(this.#config.storeConfig).forEach(c => {
          if (this.#seq[c]) {
            return done(thorin.error('SQL.CONFIG', `Store config key [${c}] already present in Sequelize.`));
          }
          this.#seq[c] = this.#config.storeConfig[c];
        });
      }
      this._extendSequelize();
      try {
        // load our models
        this.#loaded = true;
        await loader.finalizeLoading(this, this.#config, this.#models);
        ...
        logger.error(`Could not complete SQL initialization`);
        logger.debug(e);
        return done(thorin.error('SQL.INITIALIZATION', 'Could not initialize store.', e));
      }
    }
     ...
     * */
    query(stmt) {
      let seqObj = this.getInstance(), dialect = this.getDialect();
      if (!seqObj) return Promise.reject(ERROR_NOT_CONNECTED);
      if (typeof stmt === 'string' && dialect === 'sqlite') {
        let t = stmt.trim().toUpperCase(), ignores = IGNORE_QUERIES[dialect] || [];
        if (ignores.length > 0) {
          for (let i = 0, len = ignores.length; i < len; i++) {
            let ign = ignores[i];
            if (stmt.indexOf(ign) === 0) {
              logger.warn(`Skipping [${stmt}] in [${dialect}] mode`);
              return false;
            }
          }
        }
    ...
    setup(done) {
      ...
      for (let i = 0; i < SETUP_DIRECTORIES.length; i++) {
        ...
        try {
          ...
          fse.ensureDirSync(modelPath);
        } catch (e) {
        }
      }
      this.run(async (e) => {
        if (e) return done(e);
        let seqObj = this.getInstance(), opt = {
          force: true, logging: this._log.bind(this)
        };
        if (this.#config.clean !== true) opt.force = false;
        if (thorin.env === 'production') opt.force = false;
        const dialect = this.#config.options.dialect;
        if (this.#onBeforeSetup) {
          await this.#onBeforeSetup(this.#config);
          this.#onBeforeSetup = null;
        }
        try {
          if (this.#config.sync) {
            // Disable foreign key checks
            if (this._hasFkCheck()) {
              await seqObj.query('SET FOREIGN_KEY_CHECKS = 0;');
            }
            ...
            if (this.#config.clean === true && ['mariadb', 'mysql'].indexOf(dialect) !== -1) {
              let previousTables = [];
              let items = await seqObj.query('SHOW TABLES IN ' + this.#config.database);
              if (items[0]) {
              ...
              for (let i = 0, len = previousTables.length; i < len; i++) {
                let tableName = previousTables[i];
                await seqObj.query('DROP TABLE IF EXISTS `' + tableName + '`');
              }
            }
            // sync sequelize's tables.
            const _oldLogging = opt.logging, isProduction = thorin.env === 'production';
            opt.logging = (msg) => {
              if (isProduction) return;
              if (msg?.indexOf('ALTER TABLE') !== -1) return;
              if (msg?.indexOf('information_schema.tables') !== -1) return;
              if (msg?.indexOf('pg_class') !== -1) return;
              if (msg?.indexOf('pg_catalog.pg_namespace') !== -1) return;
              if (typeof _oldLogging === 'function') return _oldLogging(msg);
              logger.debug(msg);
            };
            // sync sequelize's tables.
            await seqObj.sync(opt);
          }
          // apply any patches
          ...
          if (this.#config.sync) {
            ...
            if (this._hasFkCheck()) {
              await seqObj.query('SET FOREIGN_KEY_CHECKS = 1;');
            }
          }
          ...
          logger.error('Encountered an error while setting up');
          logger.trace(e);
          return done(e);
        }
      });
    ...
    sync(opt, b) {
      ...
      if (!seqObj) {
        return Promise.reject(ERROR_NOT_CONNECTED);
      }
      // sync the entire DB
    ...
    crudify(modelName, actions, opt) {
      const modelObj = this.model(modelName), crudifyDebug = this.#config.debug.crudify;
      if (!modelObj) {
      ...
      thorin.on(thorin.EVENT.RUN, 'store.' + this.name, () => {
        actions.forEach(async (name) => {
          ...
          let dbAction = await crudify[name].call(this, actionObj, opt);
          if (!dbAction) {
          ...
          function onActionRegistered(newActionObj) {
            ...
            let logMsg = 'Crudifying "' + name + '" for model ' + modelName + ' on (' + dbAction.name + ')', aliases = [];
            if (dbAction.aliases) {
    ...
    getTransaction(opt, fn, tOpt) {
      if (typeof fn !== 'function') {
        logger.warn(`getTransaction() requires a callback function as the second argument`);
        return;
      }
      if (typeof opt === 'object' && opt && opt.transaction) {
        return fn(opt.transaction);
      }
      return this.transaction(fn, tOpt).then(function (transaction) {
     ...
     * */
    transaction(fn, opt) {
      return this.#seq.transaction(opt || {}, async (t) => {
        try {
          let r = await fn(t);
          // cleanup after commit
          t.afterCommit(function () {
            delete t._afterCommitHooks;
          });
          return r;
        } catch (e) {
          delete t._afterCommitHooks;
    ...
    /**
     * Extend Sequelize's querying capabilities with a "prepare: true"
     * Adding prepare=true and raw=true to select queries will return the raw string back.
     * */
    _extendSequelize() {
      const qi = this.#seq.queryInterface;
      const QueryTypes = this.#seq.QueryTypes;
      const _select = qi.select;
      /* Extend: select */
      qi.select = function (model, tableName, optionsArg) {
        if (optionsArg?.prepare === true && optionsArg?.raw === true) {
          const options = {
            ...optionsArg, type: QueryTypes.SELECT, model
          };
          let sql = this.queryGenerator.selectQuery(tableName, options, model);
          if (sql.charAt(sql.length - 1) === ';') sql = sql.slice(0, -1);
          return sql;
        }
    ...
    _hasFkCheck() {
      return ['mysql', 'mariadb'].indexOf(this.#config.options.dialect) !== -1;
    }
  ...
  ThorinSqlStore.Model = SqlModel;

  const initCryptoModel = require('./crypto/storeModel'), initCryptoHandler = require('./crypto/cryptor');
  ...
  loader = initLoader(thorin, ThorinSqlStore);

  return ThorinSqlStore;
};

// v3 compatibility in includes.
