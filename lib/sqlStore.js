...
const path = require('path'),
  ...
  initLoader = require('./loader'),
  initCrudify = require('./crudifyModel'),
  initModel = require('./storeModel'),
  initPatch = require('./patch/index'),
  initCrudifyAction = require('./crudifyAction'),
...
module.exports = function (thorin, opt) {
  ...
  let logger = thorin.logger(opt.name),
    crudify = initCrudify(thorin),
    CrudifyAction = initCrudifyAction(thorin),
    SqlModel = initModel(thorin),
    loader = null;
  ...
  class ThorinSqlStore extends thorin.Interface.Store {
    ...
    init(storeConfig) {
      this.#config = thorin.util.extend({
        ...
        patch: {
          tableName: '_system_version',
          delimiter: ';'    // the statement delimiter in patch files.
        },
    ...
    query(stmt) {
      ...
      if (typeof stmt === 'string' && dialect === 'sqlite') {
        let t = stmt.trim().toUpperCase(),
          ignores = IGNORE_QUERIES[dialect] || [];
        if (ignores.length > 0) {
      ...
      this.run(async (e) => {
        ...
        try {
          if (this.#config.sync) {
            ...
            const _oldLogging = opt.logging,
              isProduction = thorin.env === 'production';
            opt.logging = (msg) => {
