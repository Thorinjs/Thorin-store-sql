...
module.exports = function (thorin) {
  ...
  function parseChangeError(e, modelObj) {
    if (e && e.name.indexOf('Thorin') === 0) return e;
  ...
  actions.create = function (actionObj, opt) {
    ...
    dbAction.use((intentObj) => {
      ...
      try {
        ...
        return actionObj._runSend('create.error', intentObj);
      }
  ...
  actions.read = function (actionObj, opt) {
    ...
    dbAction.use((intentObj) => {
      ...
      try {
        ...
        intentObj.error(thorin.error(e));
        return actionObj._runSend('read.error', intentObj);
      }
      ...
      if (primaryId == null) {
        intentObj.error(thorin.error('INPUT.NOT_VALID', 'Invalid or missing ' + primaryKey.fieldName));
        return actionObj._runSend('read.error', intentObj);
      }
  ...
  actions.find = function (actionObj, opt) {
    ...
    if (opt.filters !== false) {
      ...
      modelObj.relations.forEach((relation) => {
        ...
        if (relation.options.filter === false) return;
        if (relation.type === 'belongsTo') {
    ...
    fieldValidator['order'] = dispatcher.validate('ENUM', ['asc', 'desc']).default('desc');
    ...
    dbAction.use((intentObj) => {
      ...
      if ((input.start_date || input.end_date) && !findQuery.where[modelObj.options.createdAt]) {
        if (input.start_date && input.end_date) {
          if (input.start_date > input.end_date) {
            ...
            intentObj.error(err);
            return actionObj._runSend('find.error', intentObj);
          }
      ...
      thorin.series(calls, (err) => {
        if (err) {
          intentObj.error(thorin.error(err));
          return actionObj._runSend('find.error', intentObj);
        }
        try {
  ...
  actions.update = function (actionObj, opt) {
    ...
    dbAction.use((intentObj) => {
      ...
      if (tmpObj) {
        ...
        if (!primaryId) {
          intentObj.error(thorin.error('INPUT.NOT_VALID', 'Invalid or missing ' + primaryKey.fieldName));
          return actionObj._runSend('update.error', intentObj);
        }
        try {
        ...
        calls.push((stop) => {
          return DbModel.find(findQry).then((res) => {
            if (!res) {
              stop(thorin.error('ENTRY.NOT_FOUND', 'The requested entity was not found', 404));
              return null;
            }
            resObj = res;
    ...
    dbAction.use((intentObj) => {
      ...
      if (tmpObj) {
        ...
        try {
          ...
          intentObj.error(thorin.error(e));
          return actionObj._runSend('delete.error', intentObj);
        }
        // step one, read the model.
        calls.push((stop) => {
