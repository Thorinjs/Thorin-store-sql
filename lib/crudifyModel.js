...
module.exports = function (thorin) {
  ...
  actions.find = function (actionObj, opt) {
    ...
    if (opt.filters !== false) {
      ...
      if (modelObj.options.updatedAt && opt.allowDate !== false) {
      ...
      thorin.series(calls, (err) => {
        ...
        if (opt.maxLimit !== false) {
          ...
          if (paginationData.current_page > 1) {
            paginationData.prev_page = paginationData.current_page - 1;
          }
          if (paginationData.current_page < paginationData.page_count) {
            paginationData.next_page = paginationData.current_page + 1;
          }
